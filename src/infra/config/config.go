...
type SqlDbConf struct {
	...
	MaxOpenConn         int
	MaxIdleConn         int
	MaxIdleTimeConnSecs int64
	MaxLifeTimeConnSecs int64
}
...
func Make() Config {
	...
	if err == nil {
		sqldb.MaxIdleTimeConnSecs = int64(dBMaxIdleTimeConnSeconds)
	}

	dBMaxLifeTimeConnSeconds, err := strconv.Atoi(os.Getenv("DB_MAX_LIFE_TIME_CONN_SECONDS"))
	if err == nil {
		sqldb.MaxLifeTimeConnSecs = int64(dBMaxLifeTimeConnSeconds)
	}
